...
the intervals that are random, not $p$. Saying the $p$ has a 95% chance of being between this and that is technically an incorrect statement because $p$ is not random.
...
Let’s consider the blue and red beads. are there more blue beads or red beads? I want to know if the spread $2p-1>0$

Say we take a random sample of $N=100$ and we observe $52$ blue beads, which gives us $2\bar{X.-1=0.04$. This seems to be pointing to the existence of more blue than red beads since 0.04 is larger than 0. 

We know there is chance involved in this process and we could get a 52 even when the actual spread is 0. We call the assumption that the spread is $2p-1=0$ a *null hypothesis*. The null hypothesis is the skeptic’s hypothesis. We have observed a random variable $2*\bar{X}-1=0.04$
  and the p-value is the answer to the question: how likely is it to see a value this large, when the null hypothesis is true? So we write:
$$
Pr(|\bar{X}-0.5|>0.02)
$$
...
Under this null hypothesis, we can think of this particular example as picking 4 balls out of an urn with 4 blue (correct answer) and 4 red (incorrect answer) balls.
The probability of picking 3 is 16/70.
The probability of picking all 4 correct is  1/70.
...
Computing confidence intervals for the odds ratio is not mathematically straightforward. Therefore, there is no simple way of using, for example, the CLT.
...
$$
log(\frac{ad}{bc})±1.96\sqrt{1a+1/b+1/c+1/d}
